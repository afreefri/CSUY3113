...
bool Map::IsSolid(glm::vec3 position, float* penetration_x, float* penetration_y )
{
	...
	*penetration_y = 0;
	...
	if (tile == 116 || tile == 96 || tile == 117 || tile == 137 || tile == 72 || tile == 52 || tile == 32) return false; //tree trunk and mushroom trunks 
	if (tile == 76 || tile == 56) return false; //some of the tree bushes are passable 
	if (tile == 111 || tile == 131) return false; //flat is passable 
	if (tile == 74 || tile == 54 || tile == 34 || tile == 73 || tile == 53) return false; //water is passable
